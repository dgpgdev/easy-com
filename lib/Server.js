...
var Server = function (_Emitter$EventEmitter) {
  ...
  */
	function Server() {
		var opts = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { host: 'localhost', port: 5678 };
  ...
  */
		...
		value: function initListeners() {
			...
			this.ws.on('connection', function (socket) {
				...
				socket.on('message', function (data) {
					var d = JSON.parse(data).splice(1, 0, socket);
					_this2.middlewareManager.executeMiddleware(d[0], d[1], d.slice(2), function () {
						return _this2.emit.apply(_this2, d);
					});
				});
   ...
   */
		...
		value: function start() {
			...
			this.initListeners();
			this.sendStatus((0, _extends3.default)({
				statudID: 0,
